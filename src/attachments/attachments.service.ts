...
export class AttachmentsService {
  ...
  constructor(
    ...
    @Inject(forwardRef(() => FilesService)) private fileService: FilesService,
  ...
  async createAttachments(files: any, attachableType: string, attachableId: number, transaction: Transaction): Promise<any[]> {
    return files.map( async(file, order) => 
    {
        const path =  await this.fileService.createFile(file);
        const attachment = await this.attachmentRepository.create({ path, order, attachableId, attachableType }, { transaction })
        .catch(error => 
        { 
          console.log(error);
        });
  ...
  async updateAttachments(attachmentsIds: number[], files: any, attachableType: string, attachableId: number, transaction: Transaction): Promise<any[]> {
    const attachments = await this.attachmentRepository.findAll({
      where: { attachableId, attachableType },
      order: [["order", "ASC"]]
    });
    ...
    return await files.map((file, order) => {
        this.fileService.createFile(file).then(async (path) => {
          await this.attachmentRepository.create({
            path,
            order: newFilesArrayIndexes[order],
            attachableId,
            attachableType
          }, { transaction });
        }).catch((e) => {
  ...
  async getPagedByAttachable(attachableId: number, attachableType: string, limit: number = 9, page: number = 0): Promise<any> {
    switch (attachableType) {
      case "message": {
        const dialog = await this.dialogsService.getDialogById(attachableId);
  ...
  async getAllByAttachable(attachableId: number, attachableType: string): Promise<any> {
    switch (attachableType) {
      ...
      case AttachmentScopes.POST: {
        const post = await this.postsService.getPostsById(attachableId);
        if (post) {
          return this.attachmentRepository.findAll(
            {
              where:
